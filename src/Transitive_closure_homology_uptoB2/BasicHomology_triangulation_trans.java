...
public class BasicHomology_triangulation_trans {
    ...
    int maxdimension;
    public BasicHomology_triangulation_trans(int maxdim) {
        maxdimension = maxdim;
    ...
    private void generate_representative_cycle(ExplicitSimplexStream stream,AbstractPersistenceAlgorithm<Simplex> persistence,BarcodeCollection<Double> circle_intervals) {
        ...
        try {
            ...
            */
            ...
            Iterative_trans_closure_random.bottleneckdistance_path.add(fbirdeath.getAbsolutePath());
        ...
        */
        ...
        while(m.find()){
            ...
            //System.out.println(m.groupCount());
                ...
                String the_interval_asstring = m.group().substring(1, m.group().length()-1);
                System.out.println(the_interval_asstring);
                String pattern2 = "\\[(.*?)\\)";
                Matcher m2 = Pattern.compile(pattern2).matcher(the_interval_asstring);
                
                while(m2.find()){
                     int cnt = 0;
                    while(cnt<m2.groupCount()){
                        //System.out.println("grouop count: "+m2.group(cnt));
                        String extractedsting = m2.group(cnt);
                        //System.out.println("extracted: "+extractedsting.substring(1, extractedsting.length()-1));
                        String birth_death_ar[] = extractedsting.substring(1,extractedsting.length()-1).split(",");
                        //System.out.println("array: "+birth_death_ar[0]+" "+birth_death_ar[1]);
                        if(birth_death_ar[1].trim().contains("infinity"))
