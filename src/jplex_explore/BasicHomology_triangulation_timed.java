...
public class BasicHomology_triangulation_timed {
    ...
    private static void generate_barcode_image(BarcodeCollection<Double> circle_intervals) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            try {
                ...
                Logger.getLogger(BasicHomology_triangulation_timed.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        Scanner sn = null;
        String dataset_file = "datasets/testcase_2.edges";
        ...
        while (sn.hasNextInt()) {
            ...
            if (flag[vertices[0]] == 0) {
                stream.addVertex(vertices[0],time);
                flag[vertices[0]] = 1;
            }
            if (flag[vertices[1]] == 0) {
                stream.addVertex(vertices[1],time);
                flag[vertices[1]] = 1;
            }
            stream.addElement(vertices,time);
            G.addEdge(vertices[0],vertices[1]);
            // check whether to add a face
            checkfaces(G,vertices,stream,time);
            time ++;
        }
    ...
    private static void checkfaces(AdjMatrixGraph G, int[] vertices,ExplicitSimplexStream stream, int t) {
        if(G.E()<=2) return;
            ...
            while(it.hasNext()){
                ...
                if (G.contains(adjacent_v, vertices[1])){
                    stream.addElement(new int[]{vertices[0],vertices[1],adjacent_v},t);
